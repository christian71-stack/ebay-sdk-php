...
* SDK now ignores properties in the JSON response that would normally trigger an cbdesk\eBaySDK\Exceptions\UnknownPropertyException exception.
...
* Removed constant cbdesk\eBaySDK\Marketing\Enuns\InventoryCriterionEnum::C_INVALID.
* Removed property inventoryReferenceType from cbdesk\eBaySDK\Marketing\TypesInventoryItem class.
...
* Allow properties on `cbdesk\eBaySDK\Types\DecimalType` objects to be assigned both `integer` and `double` values.
...
* The protected method `\cbdesk\eBaySDK\Services::BaseService::callOperation` has been removed. Code that called this method will now need to call `callOperationAsync(...)->wait()` instead.
...
* The class `\cbdesk\eBaySDK\Handler` is now `\cbdesk\eBaySDK\HttpHandler`.
...
* Some properties where incorrectly declared as being `cbdesk\eBaySDK\ResolutionCaseManagement\Types\Integer` instead of `integer`.
