<?php
namespace cbdesk\eBaySDK\Types;

use \cbdesk\eBaySDK\Types;
use \cbdesk\eBaySDK\Exceptions;
use \cbdesk\eBaySDK\JmesPath\Env;
use \cbdesk\eBaySDK\JmesPath\JmesPathableObjectInterface;
...
class BaseType implements JmesPathableObjectInterface
{
     ...
     *
     * @throws \cbdesk\eBaySDK\Exceptions\UnknownPropertyException If the property does not exist.
     * @throws \cbdesk\eBaySDK\Exceptions\InvalidPropertyTypeException If the value is the wrong type for the property.
     */
     ...
     * @param mixed $value. The value to assign to the property.
     *
     * @throws \cbdesk\eBaySDK\Exceptions\InvalidPropertyTypeException If trying to assign a non array type to an repeatable property.
     */
    private function setValue($class, $name, $value)
    {
        ...
        if (!$info['repeatable']) {
            ...
            if ('array' !== $actualType) {
                throw new Exceptions\InvalidPropertyTypeException($name, 'cbdesk\eBaySDK\Types\RepeatableType', $actualType);
            } else {
    ...
    private static function ensurePropertyType($class, $name, $value)
    {
        ...
        foreach ($valid as $check) {
            if ($check !== 'any' && \cbdesk\eBaySDK\checkPropertyType($check)) {
    ...
    {
        if (is_subclass_of($value, '\cbdesk\eBaySDK\Types\BaseType', false)) {
